...
export const R121_NATIVE_MENU_ROW_CONTRACT = Object.freeze({
  predecessor_index_sha256: "f51092f7915326b14a55cb0fdfaac018ef78aad54939d9384c5287486e17855e",
  predecessor_preload_name: "preload-e102b9bc.js",
  predecessor_preload_sha256: "e102b9bc5a877847b51df6e45a65c9e262412e26015227306a8f2b190cacdc4c",
  ...
  successor_preload_name: "preload-e9339fb8.js",
  successor_preload_sha256: "e9339fb81b42e829904b96e06374d4d04c56be32ada333659935ed0c051db3f2",
});
...
async function readRegularFile(target, expected, maximumBytes) {
  ...
  ) throw new Error("R121 input boundary is invalid");
  const bytes = await fs.readFile(target);
  if (sha256(bytes) !== expected) throw new Error("R121 input changed");
  return bytes;
}
...
function replaceExactlyOnce(value, anchor, replacement) {
  ...
  if (first < 0 || value.indexOf(anchor, first + anchor.length) >= 0) {
    throw new Error("R121 predecessor index anchor changed");
  }
...
export async function replaceR121NativeMenuRow({
  candidate,
  reactPreload,
  contract = R121_NATIVE_MENU_ROW_CONTRACT,
} = {}) {
  if (
    typeof candidate !== "string" || !path.isAbsolute(candidate) ||
    typeof reactPreload !== "string" || !path.isAbsolute(reactPreload)
  ) throw new Error("R121 replacement option is invalid");
  const candidateMetadata = await fs.lstat(candidate);
  if (!candidateMetadata.isDirectory() || candidateMetadata.isSymbolicLink()) {
    throw new Error("R121 candidate root is invalid");
  }

  if (!contract || typeof contract !== "object") {
    throw new Error("R121 replacement contract is invalid");
  }
  ...
  if (nextIndex.includes(Buffer.from(contract.predecessor_preload_name))) {
    throw new Error("R121 predecessor remained referenced");
  }

  const preloadTarget = path.join(assets, contract.successor_preload_name);
  const temporaryIndex = `${indexTarget}.r121-new`;
  ...
  return Object.freeze({
    event: "r121_native_menu_row_replaced",
...
function parseArguments(values) {
  ...
  if (Object.keys(options).length !== 2) {
    throw new Error("R121 replacement command is incomplete");
  }
  return options;
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  replaceR121NativeMenuRow(parseArguments(process.argv.slice(2))).then(
