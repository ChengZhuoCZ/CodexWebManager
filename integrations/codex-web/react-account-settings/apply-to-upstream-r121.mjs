...
const SOURCE_SHA256 = "e7b1d33841631dc1adeba8a86772cf702e5b23faa14256e7bb161f731d66825d";
...
const EXPECTED_PRELOAD_SHA256 = "e9339fb81b42e829904b96e06374d4d04c56be32ada333659935ed0c051db3f2";
...
async function readRegularFile(target, maximumBytes) {
  ...
  ) throw new Error("R121 upstream input boundary is invalid");
...
async function expectHash(target, expected, maximumBytes = 8 * 1024 * 1024) {
  ...
  if (sha256(bytes) !== expected) throw new Error("R121 upstream input changed");
  return bytes;
}
...
export async function applyR121NativeMenuRow({ upstreamRoot, source } = {}) {
  ...
  if (!sourceTargetMetadata.isDirectory() || sourceTargetMetadata.isSymbolicLink()) {
    throw new Error("R121 browser source root is invalid");
  }
  ...
  if (sha256(shim) !== PATCHED_SHIM_SHA256) {
    throw new Error("R121 patched shim changed");
  }
  ...
  return Object.freeze({
    event: "r121_native_menu_row_source_applied",
...
function parseArguments(values) {
  ...
  if (Object.keys(options).length !== 2) {
    throw new Error("R121 source build command is incomplete");
  }
  return options;
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  applyR121NativeMenuRow(parseArguments(process.argv.slice(2))).then(
