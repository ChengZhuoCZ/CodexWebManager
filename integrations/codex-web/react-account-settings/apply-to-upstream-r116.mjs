...
const SOURCE_SHA256 = "d59e77dc36e6a14ce239facf90cbaee62adce669a5e49b52bed71e548f615788";
...
const EXPECTED_PRELOAD_SHA256 = "e102b9bc5a877847b51df6e45a65c9e262412e26015227306a8f2b190cacdc4c";
...
async function readRegularFile(target, maximumBytes) {
  ...
  ) throw new Error("R116 upstream input boundary is invalid");
...
async function expectHash(target, expected, maximumBytes = 8 * 1024 * 1024) {
  ...
  if (sha256(bytes) !== expected) throw new Error("R116 upstream input changed");
  return bytes;
}
...
export async function applyR116ReactAccountSettings({ upstreamRoot, source } = {}) {
  ...
  if (!sourceTargetMetadata.isDirectory() || sourceTargetMetadata.isSymbolicLink()) {
    throw new Error("R116 browser source root is invalid");
  }
  ...
  if (sha256(shim) !== PATCHED_SHIM_SHA256) {
    throw new Error("R116 patched shim changed");
  }
  ...
  return Object.freeze({
    event: "r116_react_account_settings_source_applied",
...
function parseArguments(values) {
  ...
  if (Object.keys(options).length !== 2) {
    throw new Error("R116 source build command is incomplete");
  }
  return options;
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  applyR116ReactAccountSettings(parseArguments(process.argv.slice(2))).then(
