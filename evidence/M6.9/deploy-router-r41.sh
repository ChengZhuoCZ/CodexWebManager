...
readonly ARCHIVE=/tmp/codex-account-router-0.2.20-linux-x64.tar.gz
readonly ARCHIVE_SHA256=aed23645b26adde51a1240d0350f33dbd4e41a548c77599d5b9fcaa1e088cebc
readonly RELEASE_NAME=codex-account-router-0.2.20-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r41-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r41-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.20 ]]
