...
readonly RELEASE_NAME=c3e92f0f-20260801-m69-router-r82
readonly PRODUCTION_ARCHIVE=/tmp/codex-m69-r82-web-overlay.tar.gz
readonly PRODUCTION_ARCHIVE_SHA256=93957f6db55ae50fd8ddf158e9c228b0017b50d69c75764dc91e627c84654343
...
if [[ -n "$fixture_root" ]]; then
  ...
  readonly ARCHIVE=${R82_ARCHIVE:?}
  readonly ARCHIVE_SHA256=${R82_ARCHIVE_SHA256:?}
  readonly SYSTEMCTL=${R82_SYSTEMCTL:?}
  readonly REPO_ROOT=$PWD
  readonly EXPECTED_WEB_BASE_SHA256=${R82_OLD_WEB_UNIT_SHA256:?}
  readonly EXPECTED_APP_BASE_SHA256=${R82_OLD_APP_UNIT_SHA256:?}
  readonly EXPECTED_ACCOUNT_BASE_SHA256=${R82_ACCOUNT_UNIT_SHA256:?}
  readonly EXPECTED_WEB_ISOLATION_SHA256=${R82_WEB_ISOLATION_SHA256:?}
  readonly EXPECTED_APP_ISOLATION_SHA256=${R82_APP_ISOLATION_SHA256:?}
  readonly EXPECTED_ACCOUNT_ISOLATION_SHA256=${R82_ACCOUNT_ISOLATION_SHA256:?}
  readonly EXPECTED_PREVIOUS_ELECTRON_SHA256=${R82_PREVIOUS_ELECTRON_SHA256:?}
...
rollback() {
  ...
  if [[ "$success" -ne 1 ]]; then
    if [[ "$current_switched" -eq 1 ]]; then
      local rollback_link="${WEB_PREFIX}/.current-r82-rollback.$$"
...
if [[ -z "$fixture_root" ]]; then
  [[ "$current_before" == "${RELEASES}/c3e92f0f-20260801-m69-router-r81" ]]
fi
...
backup_directory=$(mktemp -d "${fixture_root:-/tmp}/m69-r82-units.XXXXXX")
...
next_link="${WEB_PREFIX}/.current-r82.$$"
...
ready=0
for attempt in $(seq 1 "${R82_READY_ATTEMPTS:-30}"); do
  ...
  sleep "${R82_READY_SLEEP_SECONDS:-0.2}"
done
[[ "$ready" -eq 1 ]]
