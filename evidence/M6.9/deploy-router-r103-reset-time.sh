...
readonly RELEASE_NAME=c3e92f0f-20260802-m69-router-r103-reset-time
readonly WEB_ROOT=/opt/0xcaff-codex-web-router
readonly WEB_CURRENT="${WEB_ROOT}/current"
readonly EXPECTED_CURRENT="${WEB_ROOT}/releases/c3e92f0f-20260802-m69-router-r102-native-size"
...
readonly INSTALLER="${R103_INSTALLER:-/tmp/replace-r103-router-reset-time.mjs}"
readonly GENERIC_INSTALLER="${R103_GENERIC_INSTALLER:-/tmp/replace-standalone-router-panel.mjs}"
readonly PANEL_SOURCE="${R103_PANEL_SOURCE:-/tmp/router-account-panel-standalone.js}"
readonly BRIDGE_SOURCE="${R103_BRIDGE_SOURCE:-/tmp/router-status-bridge-standalone.js}"
readonly QUOTA_DROPIN=/etc/systemd/system/codex-web-router.service.d/quota-refresh.conf
readonly INSTALLER_SHA256=24b62d9994a0497141c5a3dcbffe249780ac99e3a59cb59a956d46e1442706f6
readonly GENERIC_INSTALLER_SHA256=e41ef09d15848c2f32e4142558b459ccb1501036ebddd454c91e2b0f4f78ef27
readonly PANEL_SOURCE_SHA256=051747e79df1eda7dbf937ec5b930a7a30aafb97bbafb2a8df09442987fc92cc
readonly BRIDGE_SOURCE_SHA256=cec53389f8893f9ac2cf821dc2ac3a51b0d7fdd1537a3b32aba137e429515e62
...
readonly SOURCE_PANEL_ASSET=router-account-panel-131a636e.js
readonly SUCCESSOR_PANEL_ASSET=router-account-panel-051747e7.js
readonly SOURCE_INDEX_SHA256=817ba085e976df4a912e86e2bc6117a9f1f3f13ebfe9218bf1bf58df1d02a359
readonly SOURCE_INDEX_GZIP_SHA256=c43eff3b94ec40213e55f2e4916d0735204f4a25b46f01d7cca3b447f4ea8308
readonly SOURCE_INDEX_BROTLI_SHA256=f35ad6ed0137e9eb214b2201e5656e496787616d815d106089dc23f577456635
readonly SOURCE_PANEL_SHA256=131a636e367e5c2e94958bc293e73738a17b2fea2a8d05af07c8dc78bd98c098
readonly SOURCE_PANEL_GZIP_SHA256=d59d96d80cdb4a862438845d8698954560332313421a82918ef4933432e18904
readonly SOURCE_PANEL_BROTLI_SHA256=47b0f0c00cff39a61ddad203b0394640847f79e01e2ec7fe04ecfc27f4344d34
readonly SUCCESSOR_INDEX_SHA256=05efce068cbded0a87d73ad43e061064b40383dac33ff28d377f0240b65c66a2
readonly SUCCESSOR_INDEX_BROTLI_SHA256=ea2aa186aff32b8112c4c0ebca8824621dd9197d8a975ddc0aae5ec91298f24c
readonly SUCCESSOR_PANEL_BROTLI_SHA256=d3d109380f4bb263204b03d5cfdcce869c6b199656b998f8df85e4e535ab7221
...
readonly SUCCESSOR_ROUTER_BRIDGE_SHA256=cec53389f8893f9ac2cf821dc2ac3a51b0d7fdd1537a3b32aba137e429515e62
  ...
  curl --noproxy '*' -fsS --max-time 6 -X POST \
    ...
    http://127.0.0.1:8216/__backend/codex-router/quota-refresh | /usr/bin/node -e '
      ...
      if (value?.enabled!==true || value?.refreshed!==true || value?.account_alias!=="Primary" ||
          ...
          value.weekly_remaining_ratio>1 || typeof value?.weekly_resets_at!=="string" ||
          Number.isNaN(Date.parse(value.weekly_resets_at)) ||
          typeof value?.snapshot_observed_at!=="string" ||
  ...
  curl --noproxy '*' -fsS --max-time 3 -H 'Host: 100.95.50.98:8216' -H 'Accept: application/json' \
    -b "$workdir/cookies" http://127.0.0.1:8216/__backend/codex-router/status | /usr/bin/node -e '
      ...
      if (!router || router.accounts?.length!==2 || router.active_streams!==0 ||
          ...
          typeof primary?.weekly_resets_at!=="string" ||
...
rollback() {
  ...
  if [[ "$success" -ne 1 ]]; then
    if [[ "$current_switched" -eq 1 && -n "$current_before" ]]; then
      local rollback_link="${WEB_ROOT}/.current-r103-rollback.$$"
...
umask 077
workdir=$(mktemp -d /tmp/m69-r103-deploy.XXXXXX)
for unit in "$WEB_SERVICE" "$APP_SERVICE" "$ROUTER_SERVICE" "$STANDALONE_WEB_SERVICE" "$STANDALONE_APP_SERVICE"; do
...
next_link="${WEB_ROOT}/.current-r103.$$"
...
printf 'weekly_quota_reset_time_available=true\n'
