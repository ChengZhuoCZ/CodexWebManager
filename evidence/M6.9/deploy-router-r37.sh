...
readonly ARCHIVE=/tmp/codex-account-router-0.2.16-linux-x64.tar.gz
readonly ARCHIVE_SHA256=1e8cc262d3202b77e60acd0604ae6bbed24b01c3b2fe666b41e36521087dac20
readonly RELEASE_NAME=codex-account-router-0.2.16-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r37-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r37-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.16 ]]
