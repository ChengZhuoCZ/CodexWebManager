...
readonly ARCHIVE=/tmp/codex-account-router-0.2.8-linux-x64.tar.gz
readonly ARCHIVE_SHA256=8cb5614d2713a82c66799567acf3ebc727601a445e15efe1944a40c51c595876
readonly RELEASE_NAME=codex-account-router-0.2.8-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r29-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r29-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.8 ]]
