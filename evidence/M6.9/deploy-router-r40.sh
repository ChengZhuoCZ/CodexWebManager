...
readonly ARCHIVE=/tmp/codex-account-router-0.2.19-linux-x64.tar.gz
readonly ARCHIVE_SHA256=968765d6657ed5056ddd0d1f018f1f73ee5162b3706ec3b992ff0bac77a7fad5
readonly RELEASE_NAME=codex-account-router-0.2.19-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r40-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r40-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.19 ]]
