...
readonly ARCHIVE=/tmp/codex-account-router-0.2.26-linux-x64.tar.gz
readonly ARCHIVE_SHA256=f778414fcfd784d13e8dfde172b09e7c15f0a1f013e4a58a8204c7ece374280e
readonly RELEASE_NAME=codex-account-router-0.2.26-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r47-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r47-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.26 ]]
