...
readonly RELEASE_NAME=c3e92f0f-20260802-m69-router-r92-server-bridge
...
readonly QUALIFIED_SOURCE="${RELEASES}/c3e92f0f-20260802-m69-router-r91-standalone-panel"
readonly BRIDGE_SOURCE="${RELEASES}/c3e92f0f-20260801-m69-router-r86"
readonly INSTALLER="${R92_INSTALLER:-/tmp/codex-m69-r92-install-r87-router-server-bridge.mjs}"
readonly INSTALLER_SHA256=71d6d741d6bcbe6ffbe2aa403627b815c706126ee32ae300639e8b8864515bf5
readonly QUALIFIED_MAIN_SHA256=103d8dbc08f16bb3ced68d8cd0f13710dc77da2dfbed491ba95e9d510d92086d
readonly INSTALLED_MAIN_SHA256=ea69e94c622db32c8d7cc6156d9ffabd5be77ba608b3709eae4928b28c829e67
...
readonly INDEX_SHA256=f9116342677d6d51fc033fe87779f63b07ea5aca4f30a9e545b262e8a42d28da
...
readonly PANEL_SHA256=c4f3be1f8122da4f8a2f9da61054c325126af412d5cf09d4ca037e4163e058f9
readonly PRELOAD_ASSET=preload-65708a1c.js
readonly PANEL_ASSET=router-account-panel-c4f3be1f.js
...
verify_common_assets() {
  local root=$1
  [[ "$(sha256 "${root}/scratch/asar/webview/index.html")" == "$INDEX_SHA256" ]]
  [[ "$(sha256 "${root}/scratch/asar/webview/assets/app-initial-BTphDPeq.js")" == "$APP_SHA256" ]]
  [[ "$(sha256 "${root}/scratch/asar/webview/assets/${PRELOAD_ASSET}")" == "$PRELOAD_SHA256" ]]
  [[ "$(sha256 "${root}/scratch/asar/webview/assets/${PANEL_ASSET}")" == "$PANEL_SHA256" ]]
}

verify_qualified_release() {
  verify_common_assets "$QUALIFIED_SOURCE"
  [[ "$(sha256 "${QUALIFIED_SOURCE}/src/server/main.js")" == "$QUALIFIED_MAIN_SHA256" ]]
  [[ ! -e "${QUALIFIED_SOURCE}/src/server/browser-session-auth.js" ]]
  [[ ! -e "${QUALIFIED_SOURCE}/src/server/router-status-bridge.js" ]]
}

verify_successor_release() {
  verify_common_assets "$SUCCESSOR"
  [[ "$(sha256 "${SUCCESSOR}/src/server/main.js")" == "$INSTALLED_MAIN_SHA256" ]]
  [[ "$(sha256 "${SUCCESSOR}/src/server/browser-session-auth.js")" == "$SESSION_AUTH_SHA256" ]]
  [[ "$(sha256 "${SUCCESSOR}/src/server/router-status-bridge.js")" == "$ROUTER_BRIDGE_SHA256" ]]
  grep -Fq 'class WebSocketMessagePort' "${SUCCESSOR}/src/server/main.js"
  grep -Fq 'message.type === "ipc-renderer-post-message"' "${SUCCESSOR}/src/server/main.js"
}
...
validate_status_json() {
  node -e '
    ...
    if (!value || value.enabled !== true || !value.router ||
        ...
        !Array.isArray(value.router.accounts)) process.exit(1);
  '
}
...
rollback() {
  ...
  if [[ "$success" -ne 1 ]]; then
    if [[ "$current_switched" -eq 1 ]]; then
      local rollback_link="${WEB_PREFIX}/.current-r92-rollback.$$"
...
umask 077
if ! cookie_jar=$(mktemp /tmp/m69-r92-cookie.XXXXXX); then
  printf 'deployment_error=cookie_jar_create_failed\n' >&2
  exit 1
fi
...
must qualified_release_changed verify_qualified_release
...
must session_auth_source_changed test "$(sha256 "${BRIDGE_SOURCE}/src/server/browser-session-auth.js")" = "$SESSION_AUTH_SHA256"
must router_bridge_source_changed test "$(sha256 "${BRIDGE_SOURCE}/src/server/router-status-bridge.js")" = "$ROUTER_BRIDGE_SHA256"
...
must server_bridge_install_failed node "$INSTALLER" \
  --candidate "$SUCCESSOR" \
  --session-auth "${BRIDGE_SOURCE}/src/server/browser-session-auth.js" \
  --router-bridge "${BRIDGE_SOURCE}/src/server/router-status-bridge.js"
must successor_contains_symlinks test -z "$(find "$SUCCESSOR" -type l -print -quit)"
must successor_changed verify_successor_release
must standalone_8215_changed expect_8215_unchanged
must account_router_changed expect_account_router_unchanged

next_link="${WEB_PREFIX}/.current-r92.$$"
...
printf 'release=%s\n' "$RELEASE_NAME"
printf 'qualified_r23_app_host_preserved=true\n'
printf 'r86_router_server_bridge_installed=true\n'
printf 'standalone_router_panel_preserved=true\n'
