...
readonly ARCHIVE=/tmp/codex-account-router-0.2.30-linux-x64.tar.gz
readonly ARCHIVE_SHA256=e529923ee4c9ade9a714b4c74f93a1e8cf1b319dc6b42c66c09a2013b0f099aa
readonly RELEASE_NAME=codex-account-router-0.2.30-linux-x64
...
readonly STANDALONE_APP_EXPECTED_UNIT_SHA256=daaefec18fde144f1801e5b4e60a13cfe9e7c8abe98799192ee59a0f3b66db2f
readonly STANDALONE_WEB_EXPECTED_PID=510876
readonly STANDALONE_WEB_EXPECTED_ACTIVE_ENTER=196497166178
readonly STANDALONE_WEB_EXPECTED_UNIT_SHA256=543ddc71409cc088522e80651955a988cc9f2229e9da29dc8a9107c3def3ea1a
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r64-rollback.$$"
...
[[ "$standalone_app_config_before" == "$STANDALONE_APP_EXPECTED_UNIT_SHA256" ]]
...
[[ "$standalone_web_config_before" == "$STANDALONE_WEB_EXPECTED_UNIT_SHA256" ]]
...
extract_directory=$(mktemp -d /tmp/m69-r64-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.30 ]]
