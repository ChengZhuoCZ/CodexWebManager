...
readonly ARCHIVE=/tmp/codex-account-router-0.2.14-linux-x64.tar.gz
readonly ARCHIVE_SHA256=5858e573d395049d78ea520e756b3571ddad4a89f9256e971f69e23263cfe40c
readonly RELEASE_NAME=codex-account-router-0.2.14-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r35-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r35-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.14 ]]
