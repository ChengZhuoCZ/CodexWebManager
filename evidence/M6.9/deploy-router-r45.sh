...
readonly ARCHIVE=/tmp/codex-account-router-0.2.24-linux-x64.tar.gz
readonly ARCHIVE_SHA256=a0bfd27d1782508868d98a25124857120ec6fa815561477c6eacc1b77cf4da57
readonly RELEASE_NAME=codex-account-router-0.2.24-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r45-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r45-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.24 ]]
