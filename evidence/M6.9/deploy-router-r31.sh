...
readonly ARCHIVE=/tmp/codex-account-router-0.2.10-linux-x64.tar.gz
readonly ARCHIVE_SHA256=2c9bb32ad52eb04e6b9d6114c1ab245402c389956ce884bed328c1a8e06f2d20
readonly RELEASE_NAME=codex-account-router-0.2.10-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r31-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r31-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.10 ]]
