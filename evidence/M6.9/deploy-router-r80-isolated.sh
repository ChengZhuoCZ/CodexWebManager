...
readonly RELEASE_NAME=c3e92f0f-20260801-m69-router-r80
readonly PRODUCTION_ARCHIVE=/tmp/codex-m69-r80-web-overlay.tar.gz
...
if [[ -n "$fixture_root" ]]; then
  ...
  readonly ARCHIVE=${R80_ARCHIVE:?}
  readonly ARCHIVE_SHA256=${R80_ARCHIVE_SHA256:?}
  readonly SYSTEMCTL=${R80_SYSTEMCTL:?}
  readonly REPO_ROOT=$PWD
  readonly EXPECTED_WEB_BASE_SHA256=${R80_OLD_WEB_UNIT_SHA256:?}
  readonly EXPECTED_APP_BASE_SHA256=${R80_OLD_APP_UNIT_SHA256:?}
  readonly EXPECTED_ACCOUNT_BASE_SHA256=${R80_ACCOUNT_UNIT_SHA256:?}
  readonly EXPECTED_WEB_ISOLATION_SHA256=${R80_WEB_ISOLATION_SHA256:?}
  readonly EXPECTED_APP_ISOLATION_SHA256=${R80_APP_ISOLATION_SHA256:?}
  readonly EXPECTED_ACCOUNT_ISOLATION_SHA256=${R80_ACCOUNT_ISOLATION_SHA256:?}
  readonly EXPECTED_PREVIOUS_ELECTRON_SHA256=${R80_PREVIOUS_ELECTRON_SHA256:?}
...
rollback() {
  ...
  if [[ "$success" -ne 1 ]]; then
    if [[ "$current_switched" -eq 1 ]]; then
      local rollback_link="${WEB_PREFIX}/.current-r80-rollback.$$"
...
backup_directory=$(mktemp -d "${fixture_root:-/tmp}/m69-r80-units.XXXXXX")
...
next_link="${WEB_PREFIX}/.current-r80.$$"
...
ready=0
for attempt in $(seq 1 "${R80_READY_ATTEMPTS:-30}"); do
  ...
  sleep "${R80_READY_SLEEP_SECONDS:-0.2}"
done
[[ "$ready" -eq 1 ]]
