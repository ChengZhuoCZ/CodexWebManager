...
readonly ARCHIVE=/tmp/codex-account-router-0.2.28-linux-x64.tar.gz
readonly ARCHIVE_SHA256=5f8b44a259cade5286b8a35ff6354e380760938b2c492e64b94bc2fefa560f5c
readonly RELEASE_NAME=codex-account-router-0.2.28-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r49-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r49-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.28 ]]
