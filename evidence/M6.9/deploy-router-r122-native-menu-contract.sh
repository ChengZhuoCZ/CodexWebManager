...
readonly RELEASE_NAME=c3e92f0f-20260803-m69-router-r122-native-menu-contract
readonly WEB_ROOT=/opt/0xcaff-codex-web-router
readonly WEB_CURRENT="${WEB_ROOT}/current"
readonly EXPECTED_CURRENT="${WEB_ROOT}/releases/c3e92f0f-20260803-m69-router-r121-native-menu-row"
...
readonly INPUT_ROOT="${R122_INPUT_ROOT:-/tmp/codex-r122-deploy-inputs}"
readonly TRANSFORMER="${INPUT_ROOT}/replace-r122-native-menu-contract.mjs"
readonly PRELOAD="${INPUT_ROOT}/preload-r122.js"
readonly TRANSFORMER_SHA256=c58f7bde835b1b85904372cec08f53bab184a30c3f1389db02b0b85efced89b0
readonly PRELOAD_SHA256=ba57f3d68765fd06d0830c8d1bae5d01c35b02835f71624607733dd869d82dfb

readonly INDEX=scratch/asar/webview/index.html
readonly ASSETS=scratch/asar/webview/assets
readonly OLD_PRELOAD=preload-e9339fb8.js
readonly NEW_PRELOAD=preload-ba57f3d6.js
readonly CONTROLLER=router-account-controller-041ab79a.js
readonly APP=app-initial-BTphDPeq.js
readonly OLD_INDEX_SHA256=731d8f71bc1dca7ccd359c21d391530f8da4d53955fecaf5bbf8575045f37441
readonly OLD_INDEX_GZIP_SHA256=b50329e32054184ff8ce338dd743be06727c7c8c693a1b7e87d0ac808dff10c1
readonly OLD_INDEX_BROTLI_SHA256=ed6e0de9f564a3652a616b0123a63e7881c4860560e6f7951f6c8ad046747362
readonly NEW_INDEX_SHA256=ca1bd8b371a3001ef9bdaa5764c5da1a57b27206d2c7a10121a98791f02464d0
readonly OLD_PRELOAD_SHA256=e9339fb81b42e829904b96e06374d4d04c56be32ada333659935ed0c051db3f2
readonly OLD_PRELOAD_GZIP_SHA256=04d4ffeb66fb03d07b7cf48ad2b65716832ae41aca4f4a908acaebf1d1c3f9ab
readonly OLD_PRELOAD_BROTLI_SHA256=c107c34f1fb1f959181d7f06cbd65f7ab94d01d616b57d5d5c5554ff03d5a7f7
...
verify_successor() {
  ...
  grep -Fq 'native-contract' "${SUCCESSOR}/${ASSETS}/${NEW_PRELOAD}" || return 1
  grep -Fq 'flex w-full items-center gap-1.5' "${SUCCESSOR}/${ASSETS}/${NEW_PRELOAD}" || return 1
  grep -Fq 'ml-2 shrink-0 text-xs text-token-description-foreground' "${SUCCESSOR}/${ASSETS}/${NEW_PRELOAD}" || return 1
...
rollback() {
  ...
  if [[ "$success" -ne 1 ]]; then
    if [[ "$current_switched" -eq 1 && -n "$current_before" ]]; then
      local rollback_link="${WEB_ROOT}/.current-r122-rollback.$$"
...
umask 077
workdir=$(mktemp -d /tmp/m69-r122-deploy.XXXXXX)
for unit in "$WEB_SERVICE" "$APP_SERVICE" "$ROUTER_SERVICE" "$MANAGER_SERVICE" \
  "$MANAGER_SOCKET" "$STANDALONE_WEB_SERVICE" "$STANDALONE_APP_SERVICE"; do
...
next_link="${WEB_ROOT}/.current-r122-next.$$"
...
printf 'release=%s\n' "$RELEASE_NAME"
printf 'native_menu_layout=native-contract\n'
