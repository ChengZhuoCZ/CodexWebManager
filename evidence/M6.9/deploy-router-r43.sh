...
readonly ARCHIVE=/tmp/codex-account-router-0.2.22-linux-x64.tar.gz
readonly ARCHIVE_SHA256=a2432050f9ff5b09e855c0358fcfd615ba8e79823118f5bb007688d12dd34625
readonly RELEASE_NAME=codex-account-router-0.2.22-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r43-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r43-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.22 ]]
