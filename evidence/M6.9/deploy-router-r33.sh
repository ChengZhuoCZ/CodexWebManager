...
readonly ARCHIVE=/tmp/codex-account-router-0.2.12-linux-x64.tar.gz
readonly ARCHIVE_SHA256=169a6b87cf795ef771bbd537c2b49ab986531aca417d0e600b293bf0b6611725
readonly RELEASE_NAME=codex-account-router-0.2.12-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r33-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r33-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.12 ]]
