...
readonly ARCHIVE=/tmp/codex-account-router-0.2.15-linux-x64.tar.gz
readonly ARCHIVE_SHA256=186837d746095e94790b66491ad8014ff8b51ce3d5b648e3bb0a4bcef369a79e
readonly RELEASE_NAME=codex-account-router-0.2.15-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r36-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r36-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.15 ]]
