...
readonly ARCHIVE=/tmp/codex-account-router-0.2.21-linux-x64.tar.gz
readonly ARCHIVE_SHA256=c52bc172829543434c152d132bddc98d72369aff2260c262bd349a4a5a302a16
readonly RELEASE_NAME=codex-account-router-0.2.21-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r42-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r42-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.21 ]]
