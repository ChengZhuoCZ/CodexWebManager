...
readonly RELEASE_NAME=c3e92f0f-20260802-m69-router-r101-quota-menu
readonly WEB_ROOT=/opt/0xcaff-codex-web-router
readonly WEB_CURRENT="${WEB_ROOT}/current"
readonly EXPECTED_CURRENT="${WEB_ROOT}/releases/c3e92f0f-20260802-m69-router-r100-profile-menu-dom"
...
readonly INSTALLER="${R101_INSTALLER:-/tmp/replace-r101-router-quota-menu.mjs}"
readonly GENERIC_INSTALLER="${R101_GENERIC_INSTALLER:-/tmp/replace-standalone-router-panel.mjs}"
readonly PANEL_SOURCE="${R101_PANEL_SOURCE:-/tmp/router-account-panel-standalone.js}"
readonly BRIDGE_SOURCE="${R101_BRIDGE_SOURCE:-/tmp/router-status-bridge-standalone.js}"
readonly QUOTA_DROPIN_SOURCE="${R101_QUOTA_DROPIN_SOURCE:-/tmp/codex-web-router-quota-refresh.conf}"
readonly QUOTA_DROPIN=/etc/systemd/system/codex-web-router.service.d/quota-refresh.conf
readonly INSTALLER_SHA256=4983a7ef358ece1760c62df2cb5083ff0a126f14dec8cd4b76e897db9d497e4a
readonly GENERIC_INSTALLER_SHA256=e41ef09d15848c2f32e4142558b459ccb1501036ebddd454c91e2b0f4f78ef27
readonly PANEL_SOURCE_SHA256=fb575a578aedb851e7892be7b1a98e0fc529352e7fd3199c2947acae5688e468
readonly BRIDGE_SOURCE_SHA256=ac5c1f13c23df530de59a4d396f1b6faeab0f7f9d6d57c1aa1d1a2b7500fc69e
...
readonly SOURCE_PANEL_ASSET=router-account-panel-559921ae.js
readonly SUCCESSOR_PANEL_ASSET=router-account-panel-fb575a57.js
readonly SOURCE_INDEX_SHA256=3d868b8b58ef8e936e082c481305aeb77a81978623e58063c5f6816309e37ab6
readonly SOURCE_INDEX_GZIP_SHA256=9278326d0ef18ecaf9ae0a48aa7fe8e0faf2ac7f8461fd418a2f025c8f68491c
readonly SOURCE_INDEX_BROTLI_SHA256=e9f8d5bf3cbcffc68238e547a6af072a934b267c488dc6590be504191da28c44
readonly SOURCE_PANEL_SHA256=559921aef674c12d7f84279c6153d58b8abe0635a64ae6d9ab786499159a3bcd
readonly SOURCE_PANEL_GZIP_SHA256=d960d13ffe5abd0483296238e8e528d6f87bff8cc03dd9097729cc5340e85526
readonly SOURCE_PANEL_BROTLI_SHA256=1cfd96adfa75c6d62d639199ae6f776458a73041d11451a4643d804fbf1acae2
readonly SUCCESSOR_INDEX_SHA256=c506a647624fc65d84de7b2209679c08ad7123d9d84999e65bc8f4acdacae03b
readonly SUCCESSOR_INDEX_BROTLI_SHA256=54e2fda71ef23bce22ea276a38b4dc39a61ad628293df87caf665f50454894cf
readonly SUCCESSOR_PANEL_BROTLI_SHA256=20e0831b1b3727a1549b26251412d84e64ec6336c87d75d412e45774f674700d
...
readonly SOURCE_ROUTER_BRIDGE_SHA256=185cf83adf11510e2b5c69d3c0652f5c146aa87a56ed97c6028ea2dc62bf3757
...
verify_successor() {
  verify_common_assets "$SUCCESSOR" "$BRIDGE_SOURCE_SHA256" || return 1
...
rollback() {
  ...
  if [[ "$success" -ne 1 ]]; then
    ...
    if [[ "$current_switched" -eq 1 && -n "$current_before" ]]; then
      local rollback_link="${WEB_ROOT}/.current-r101-rollback.$$"
...
umask 077
workdir=$(mktemp -d /tmp/m69-r101-deploy.XXXXXX)
for unit in "$WEB_SERVICE" "$APP_SERVICE" "$ROUTER_SERVICE" "$STANDALONE_WEB_SERVICE" "$STANDALONE_APP_SERVICE"; do
...
must bridge_changed test "$(sha256 "$BRIDGE_SOURCE")" = "$BRIDGE_SOURCE_SHA256"
must quota_dropin_changed test "$(sha256 "$QUOTA_DROPIN_SOURCE")" = "$QUOTA_DROPIN_SHA256"
must quota_dropin_exists test ! -e "$QUOTA_DROPIN"
...
must bridge_replace install -o root -g root -m 0644 \
  "$BRIDGE_SOURCE" "$SUCCESSOR/src/server/router-status-bridge.js"
must successor_symlinks test -z "$(find "$SUCCESSOR" -type l -print -quit)"
must successor_changed verify_successor
must protected_changed verify_protected

must quota_dropin_install install -o root -g root -m 0644 "$QUOTA_DROPIN_SOURCE" "$QUOTA_DROPIN"
dropin_installed=1
...
next_link="${WEB_ROOT}/.current-r101.$$"
