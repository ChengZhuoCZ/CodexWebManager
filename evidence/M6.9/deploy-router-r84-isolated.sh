...
readonly RELEASE_NAME=c3e92f0f-20260801-m69-router-r84
readonly PRODUCTION_ARCHIVE=/tmp/codex-m69-r84-web-overlay.tar.gz
readonly PRODUCTION_ARCHIVE_SHA256=4edf04cbd438553f90717fb7b923884f8eabc11d2376f01b10560c7edbce67c3
...
if [[ -n "$fixture_root" ]]; then
  ...
  readonly ARCHIVE=${R84_ARCHIVE:?}
  readonly ARCHIVE_SHA256=${R84_ARCHIVE_SHA256:?}
  readonly SYSTEMCTL=${R84_SYSTEMCTL:?}
  readonly REPO_ROOT=$PWD
  readonly EXPECTED_WEB_BASE_SHA256=${R84_OLD_WEB_UNIT_SHA256:?}
  readonly EXPECTED_APP_BASE_SHA256=${R84_OLD_APP_UNIT_SHA256:?}
  readonly EXPECTED_ACCOUNT_BASE_SHA256=${R84_ACCOUNT_UNIT_SHA256:?}
  readonly EXPECTED_WEB_ISOLATION_SHA256=${R84_WEB_ISOLATION_SHA256:?}
  readonly EXPECTED_APP_ISOLATION_SHA256=${R84_APP_ISOLATION_SHA256:?}
  readonly EXPECTED_ACCOUNT_ISOLATION_SHA256=${R84_ACCOUNT_ISOLATION_SHA256:?}
  readonly EXPECTED_PREVIOUS_ELECTRON_SHA256=${R84_PREVIOUS_ELECTRON_SHA256:?}
...
rollback() {
  ...
  if [[ "$success" -ne 1 ]]; then
    if [[ "$current_switched" -eq 1 ]]; then
      local rollback_link="${WEB_PREFIX}/.current-r84-rollback.$$"
...
if [[ -z "$fixture_root" ]]; then
  [[ "$current_before" == "${RELEASES}/c3e92f0f-20260801-m69-router-r83" ]]
fi
...
backup_directory=$(mktemp -d "${fixture_root:-/tmp}/m69-r84-units.XXXXXX")
...
next_link="${WEB_PREFIX}/.current-r84.$$"
...
ready=0
for attempt in $(seq 1 "${R84_READY_ATTEMPTS:-30}"); do
  ...
  sleep "${R84_READY_SLEEP_SECONDS:-0.2}"
done
[[ "$ready" -eq 1 ]]
