...
readonly ARCHIVE=/tmp/codex-account-router-0.2.13-linux-x64.tar.gz
readonly ARCHIVE_SHA256=1fd7a4f6a3da250f538120633c819057cf527a2dda3831909f401cbf0605b82d
readonly RELEASE_NAME=codex-account-router-0.2.13-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r34-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r34-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.13 ]]
