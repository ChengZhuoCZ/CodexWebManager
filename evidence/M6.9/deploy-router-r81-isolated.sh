...
readonly RELEASE_NAME=c3e92f0f-20260801-m69-router-r81
readonly PRODUCTION_ARCHIVE=/tmp/codex-m69-r81-web-overlay.tar.gz
readonly PRODUCTION_ARCHIVE_SHA256=93e21e39ed6f51c8addf4045f44bc5fbf4c50376585f69d83d97d4312855e731
...
if [[ -n "$fixture_root" ]]; then
  ...
  readonly ARCHIVE=${R81_ARCHIVE:?}
  readonly ARCHIVE_SHA256=${R81_ARCHIVE_SHA256:?}
  readonly SYSTEMCTL=${R81_SYSTEMCTL:?}
  readonly REPO_ROOT=$PWD
  readonly EXPECTED_WEB_BASE_SHA256=${R81_OLD_WEB_UNIT_SHA256:?}
  readonly EXPECTED_APP_BASE_SHA256=${R81_OLD_APP_UNIT_SHA256:?}
  readonly EXPECTED_ACCOUNT_BASE_SHA256=${R81_ACCOUNT_UNIT_SHA256:?}
  readonly EXPECTED_WEB_ISOLATION_SHA256=${R81_WEB_ISOLATION_SHA256:?}
  readonly EXPECTED_APP_ISOLATION_SHA256=${R81_APP_ISOLATION_SHA256:?}
  readonly EXPECTED_ACCOUNT_ISOLATION_SHA256=${R81_ACCOUNT_ISOLATION_SHA256:?}
  readonly EXPECTED_PREVIOUS_ELECTRON_SHA256=${R81_PREVIOUS_ELECTRON_SHA256:?}
...
rollback() {
  ...
  if [[ "$success" -ne 1 ]]; then
    if [[ "$current_switched" -eq 1 ]]; then
      local rollback_link="${WEB_PREFIX}/.current-r81-rollback.$$"
...
if [[ -z "$fixture_root" ]]; then
  [[ "$current_before" == "${RELEASES}/c3e92f0f-20260801-m69-router-r80" ]]
fi
...
backup_directory=$(mktemp -d "${fixture_root:-/tmp}/m69-r81-units.XXXXXX")
...
next_link="${WEB_PREFIX}/.current-r81.$$"
...
ready=0
for attempt in $(seq 1 "${R81_READY_ATTEMPTS:-30}"); do
  ...
  sleep "${R81_READY_SLEEP_SECONDS:-0.2}"
done
[[ "$ready" -eq 1 ]]
