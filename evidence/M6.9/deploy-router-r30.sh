...
readonly ARCHIVE=/tmp/codex-account-router-0.2.9-linux-x64.tar.gz
readonly ARCHIVE_SHA256=60b2341e691b8103ec03bba7ca2e4458ef0dc03441a75fde40d5f9bc2001375d
readonly RELEASE_NAME=codex-account-router-0.2.9-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r30-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r30-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.9 ]]
