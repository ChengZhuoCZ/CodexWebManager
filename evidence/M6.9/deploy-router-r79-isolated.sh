...
readonly RELEASE_NAME=c3e92f0f-20260801-m69-router-r79
readonly PRODUCTION_ARCHIVE=/tmp/codex-m69-r79-web-overlay.tar.gz
readonly PRODUCTION_ARCHIVE_SHA256=ed1b420c29ab9ccbf10fa4249d6c9d221584d77bf43f067e2bff29731c122199
...
if [[ -n "$fixture_root" ]]; then
  ...
  readonly ARCHIVE=${R79_ARCHIVE:?}
  readonly ARCHIVE_SHA256=${R79_ARCHIVE_SHA256:?}
  readonly SYSTEMCTL=${R79_SYSTEMCTL:?}
  readonly REPO_ROOT=$PWD
  readonly EXPECTED_WEB_BASE_SHA256=${R79_OLD_WEB_UNIT_SHA256:?}
  readonly EXPECTED_APP_BASE_SHA256=${R79_OLD_APP_UNIT_SHA256:?}
  readonly EXPECTED_ACCOUNT_BASE_SHA256=${R79_ACCOUNT_UNIT_SHA256:?}
  readonly EXPECTED_WEB_ISOLATION_SHA256=${R79_WEB_ISOLATION_SHA256:?}
  readonly EXPECTED_APP_ISOLATION_SHA256=${R79_APP_ISOLATION_SHA256:?}
  readonly EXPECTED_ACCOUNT_ISOLATION_SHA256=${R79_ACCOUNT_ISOLATION_SHA256:?}
  readonly EXPECTED_PREVIOUS_ELECTRON_SHA256=${R79_PREVIOUS_ELECTRON_SHA256:?}
...
rollback() {
  ...
  if [[ "$success" -ne 1 ]]; then
    if [[ "$current_switched" -eq 1 ]]; then
      local rollback_link="${WEB_PREFIX}/.current-r79-rollback.$$"
...
backup_directory=$(mktemp -d "${fixture_root:-/tmp}/m69-r79-units.XXXXXX")
...
next_link="${WEB_PREFIX}/.current-r79.$$"
...
ready=0
for attempt in $(seq 1 "${R79_READY_ATTEMPTS:-30}"); do
  ...
  sleep "${R79_READY_SLEEP_SECONDS:-0.2}"
done
[[ "$ready" -eq 1 ]]
