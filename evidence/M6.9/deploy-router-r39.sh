...
readonly ARCHIVE=/tmp/codex-account-router-0.2.18-linux-x64.tar.gz
readonly ARCHIVE_SHA256=32cfe6dd8e8e23437e77ce7cb4506639c148050a850951d76db71f657bc6a3fd
readonly RELEASE_NAME=codex-account-router-0.2.18-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r39-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r39-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.18 ]]
