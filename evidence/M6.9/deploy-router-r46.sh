...
readonly ARCHIVE=/tmp/codex-account-router-0.2.25-linux-x64.tar.gz
readonly ARCHIVE_SHA256=4f3f567d1f691bf49b1b4ab77d16c22edf35070f29770666d05577c7052ac178
readonly RELEASE_NAME=codex-account-router-0.2.25-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r46-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r46-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.25 ]]
