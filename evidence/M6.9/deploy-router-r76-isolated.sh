...
readonly RELEASE_NAME=c3e92f0f-20260801-m69-router-r76
...
readonly WEB_UNIT_SHA256=6bf41fbfe8ed66508a088eac9cc49cc6af623aa76cecc9a956189a3893e51dd1
readonly APP_UNIT_SHA256=3f84a2d964a6361ece8e75a21344651d6790cad2f5219b64afe1aab93d8cfabd
...
if [[ -n "$fixture_root" ]]; then
  ...
  readonly ARCHIVE=${R76_ARCHIVE:?}
  readonly ARCHIVE_SHA256=${R76_ARCHIVE_SHA256:?}
  readonly SYSTEMCTL=${R76_SYSTEMCTL:?}
  readonly REPO_ROOT=$PWD
  readonly EXPECTED_WEB_BASE_SHA256=${R76_OLD_WEB_UNIT_SHA256:?}
  readonly EXPECTED_APP_BASE_SHA256=${R76_OLD_APP_UNIT_SHA256:?}
  readonly EXPECTED_ACCOUNT_BASE_SHA256=${R76_ACCOUNT_UNIT_SHA256:?}
  readonly EXPECTED_WEB_ISOLATION_SHA256=${R76_WEB_ISOLATION_SHA256:?}
  readonly EXPECTED_APP_ISOLATION_SHA256=${R76_APP_ISOLATION_SHA256:?}
  readonly EXPECTED_ACCOUNT_ISOLATION_SHA256=${R76_ACCOUNT_ISOLATION_SHA256:?}
...
rollback() {
  ...
  if [[ "$success" -ne 1 ]]; then
    if [[ "$current_switched" -eq 1 ]]; then
      local rollback_link="${WEB_PREFIX}/.current-r76-rollback.$$"
...
backup_directory=$(mktemp -d "${fixture_root:-/tmp}/m69-r76-units.XXXXXX")
...
next_link="${WEB_PREFIX}/.current-r76.$$"
...
ready=0
for attempt in $(seq 1 "${R76_READY_ATTEMPTS:-30}"); do
  ...
  sleep "${R76_READY_SLEEP_SECONDS:-0.2}"
done
[[ "$ready" -eq 1 ]]
