...
readonly ARCHIVE=/tmp/codex-account-router-0.2.17-linux-x64.tar.gz
readonly ARCHIVE_SHA256=859a3d89ef2acb95cbeb693c4af036f17fdacd463841d63b8803199c018ab84f
readonly RELEASE_NAME=codex-account-router-0.2.17-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r38-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r38-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.17 ]]
