...
readonly ARCHIVE=/tmp/codex-account-router-0.2.23-linux-x64.tar.gz
readonly ARCHIVE_SHA256=5541978fe685459dcaf6fef4ea3d7bb9d95697ce38cb69a4dae22c5a4148d177
readonly RELEASE_NAME=codex-account-router-0.2.23-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r44-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r44-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.23 ]]
