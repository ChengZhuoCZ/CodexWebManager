...
readonly ARCHIVE=/tmp/codex-account-router-0.2.6-linux-x64.tar.gz
readonly ARCHIVE_SHA256=718bbf6f24d9c2c742c064754af622501f05a3230c6a2670e6f7c60fce932c04
readonly RELEASE_NAME=codex-account-router-0.2.6-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r24-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r24-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.6 ]]
