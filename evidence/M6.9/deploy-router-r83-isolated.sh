...
readonly RELEASE_NAME=c3e92f0f-20260801-m69-router-r83
readonly PRODUCTION_ARCHIVE=/tmp/codex-m69-r83-web-overlay.tar.gz
readonly PRODUCTION_ARCHIVE_SHA256=bcef90232ae8a6f81c9785705fb0cadc701c330983ad7712991905f2e1aa479b
readonly PREVIOUS_ELECTRON_SHA256=51e9a0bcbd38ec5f7c57c75b1ff668047fb219cb17e6ce51c2571c40d0aa4bf4
...
readonly OLD_WEB_UNIT_SHA256=5aa4a2bf9951c90a871963b7a683d6922e5906967fa0de698ae199bf1f5b0159
readonly OLD_APP_UNIT_SHA256=983b71a39bec32bdb1c9c74f0cd203f4c70e4f08ca4c8549df87fe8e29b141ad
...
if [[ -n "$fixture_root" ]]; then
  ...
  readonly ARCHIVE=${R83_ARCHIVE:?}
  readonly ARCHIVE_SHA256=${R83_ARCHIVE_SHA256:?}
  readonly SYSTEMCTL=${R83_SYSTEMCTL:?}
  readonly REPO_ROOT=$PWD
  readonly EXPECTED_WEB_BASE_SHA256=${R83_OLD_WEB_UNIT_SHA256:?}
  readonly EXPECTED_APP_BASE_SHA256=${R83_OLD_APP_UNIT_SHA256:?}
  readonly EXPECTED_ACCOUNT_BASE_SHA256=${R83_ACCOUNT_UNIT_SHA256:?}
  readonly EXPECTED_WEB_ISOLATION_SHA256=${R83_WEB_ISOLATION_SHA256:?}
  readonly EXPECTED_APP_ISOLATION_SHA256=${R83_APP_ISOLATION_SHA256:?}
  readonly EXPECTED_ACCOUNT_ISOLATION_SHA256=${R83_ACCOUNT_ISOLATION_SHA256:?}
  readonly EXPECTED_PREVIOUS_ELECTRON_SHA256=${R83_PREVIOUS_ELECTRON_SHA256:?}
  ...
  readonly EXPECTED_PREVIOUS_ELECTRON_SHA256=$PREVIOUS_ELECTRON_SHA256
...
rollback() {
  ...
  if [[ "$success" -ne 1 ]]; then
    if [[ "$current_switched" -eq 1 ]]; then
      local rollback_link="${WEB_PREFIX}/.current-r83-rollback.$$"
...
if [[ -z "$fixture_root" ]]; then
  [[ "$current_before" == "${RELEASES}/c3e92f0f-20260801-m69-router-r82" ]]
fi
...
backup_directory=$(mktemp -d "${fixture_root:-/tmp}/m69-r83-units.XXXXXX")
...
next_link="${WEB_PREFIX}/.current-r83.$$"
...
ready=0
for attempt in $(seq 1 "${R83_READY_ATTEMPTS:-30}"); do
  ...
  sleep "${R83_READY_SLEEP_SECONDS:-0.2}"
done
[[ "$ready" -eq 1 ]]
