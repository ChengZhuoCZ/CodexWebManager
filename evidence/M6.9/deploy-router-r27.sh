...
readonly ARCHIVE=/tmp/codex-account-router-0.2.7-linux-x64.tar.gz
readonly ARCHIVE_SHA256=eeb4ff069d083bb0259ec82a4d1ab4ab16edb5747bdc523f7a941883529ea8a4
readonly RELEASE_NAME=codex-account-router-0.2.7-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r27-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r27-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.7 ]]
