...
readonly ARCHIVE=/tmp/codex-account-router-0.2.11-linux-x64.tar.gz
readonly ARCHIVE_SHA256=722723f04c169ba1f73e2ba10aea6f69887dd173a76a1209552780f9b6e99da0
readonly RELEASE_NAME=codex-account-router-0.2.11-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r32-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r32-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.11 ]]
