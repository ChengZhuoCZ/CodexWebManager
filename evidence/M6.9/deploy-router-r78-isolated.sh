...
readonly RELEASE_NAME=c3e92f0f-20260801-m69-router-r78
readonly PRODUCTION_ARCHIVE=/tmp/codex-m69-r78-web-overlay.tar.gz
readonly PRODUCTION_ARCHIVE_SHA256=a742fb897a3d954d79328317cf22381cf7fe440c98b81c70eef0fcb8ad29940c
...
if [[ -n "$fixture_root" ]]; then
  ...
  readonly ARCHIVE=${R78_ARCHIVE:?}
  readonly ARCHIVE_SHA256=${R78_ARCHIVE_SHA256:?}
  readonly SYSTEMCTL=${R78_SYSTEMCTL:?}
  readonly REPO_ROOT=$PWD
  readonly EXPECTED_WEB_BASE_SHA256=${R78_OLD_WEB_UNIT_SHA256:?}
  readonly EXPECTED_APP_BASE_SHA256=${R78_OLD_APP_UNIT_SHA256:?}
  readonly EXPECTED_ACCOUNT_BASE_SHA256=${R78_ACCOUNT_UNIT_SHA256:?}
  readonly EXPECTED_WEB_ISOLATION_SHA256=${R78_WEB_ISOLATION_SHA256:?}
  readonly EXPECTED_APP_ISOLATION_SHA256=${R78_APP_ISOLATION_SHA256:?}
  readonly EXPECTED_ACCOUNT_ISOLATION_SHA256=${R78_ACCOUNT_ISOLATION_SHA256:?}
...
rollback() {
  ...
  if [[ "$success" -ne 1 ]]; then
    if [[ "$current_switched" -eq 1 ]]; then
      local rollback_link="${WEB_PREFIX}/.current-r78-rollback.$$"
...
backup_directory=$(mktemp -d "${fixture_root:-/tmp}/m69-r78-units.XXXXXX")
...
next_link="${WEB_PREFIX}/.current-r78.$$"
...
ready=0
for attempt in $(seq 1 "${R78_READY_ATTEMPTS:-30}"); do
  ...
  sleep "${R78_READY_SLEEP_SECONDS:-0.2}"
done
[[ "$ready" -eq 1 ]]
