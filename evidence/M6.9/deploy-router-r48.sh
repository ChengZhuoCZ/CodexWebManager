...
readonly ARCHIVE=/tmp/codex-account-router-0.2.27-linux-x64.tar.gz
readonly ARCHIVE_SHA256=6047a6c1707d4b73fdf6b870ac5734499e721a19567f62c67daee888f77394c8
readonly RELEASE_NAME=codex-account-router-0.2.27-linux-x64
...
rollback() {
  ...
  if [[ "$success" -ne 1 && "$current_switched" -eq 1 && -n "$previous_release" ]]; then
    local rollback_link="${ROUTER_ROOT}/.current-r48-rollback.$$"
...
extract_directory=$(mktemp -d /tmp/m69-r48-deploy.XXXXXX)
...
[[ "$(node -p "require('${extracted_release}/lib/account-router/package.json').version")" == 0.2.27 ]]
