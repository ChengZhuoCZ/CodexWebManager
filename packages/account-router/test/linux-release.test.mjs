...
test("builds a byte-reproducible x64 Linux archive without desktop dependencies", async (context) => {
  ...
  assert.equal(manifest.version, "0.2.38");
  assert.equal(first.releaseName, "codex-account-router-0.2.38-linux-x64");
  ...
  assert.ok(archivedPaths.includes(`${root}/bin/codex-router-account-manager`));
  ...
  for (const executablePath of [
    ...
    `${root}/bin/codex-router-account-manager`,
...
test(
  ...
  assert.deepEqual(
    summary.runtime.synthetic_websocket_all_pool_unavailable,
    {
      configured_bindings: 2,
      process_starts: 1,
  ...
  assert.deepEqual(summary.runtime.synthetic_retry_backoff_deadline, {
    configured_bindings: 2,
    scenarios: 2,
    process_starts: 2,
    readiness_statuses: [200, 200],
