...
In the production bootstrap path, the account metadata, optional admin authenticator, circuit state,
and routing state stages also share one outer five-second total deadline. The stage-specific
...
The packaged systemd unit permits up to 16 distinct account attempts, with a 120-second total
deadline and 100–2000 ms backoff. This lets the router reach every supported configured account
without removing the independent deadline or backoff bounds. Startup fails closed if a value is
malformed, outside the state-machine ceiling, or if the base backoff exceeds the maximum.
